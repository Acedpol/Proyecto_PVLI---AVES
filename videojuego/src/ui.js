...
export default class UI extends Phaser.Scene {
  ...
  create() {
    ...
    this.timerText = this.add.text(this.sys.game.config.width / 2 - 10, 30, this.timer.getDuration(), {
      fontSize: 40,
      align: 'center'
    });
    this.woodIcon = this.add.image(this.sys.game.config.width - 80, 45, 'wood').setScale(4);
    this.woodText = this.add.text(this.sys.game.config.width - 50, 30, this.player.getWood(), {
      fontSize: 40,
      align: 'right'
    })
